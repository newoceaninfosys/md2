...
export class ConnectedPositionStrategy implements PositionStrategy {
  ...
  _onPositionChange: Subject<ConnectedOverlayPositionChange> = new Subject<
    ConnectedOverlayPositionChange
  >();
  ...
  apply(element: HTMLElement): Promise<void> {
    ...
    for (let pos of this._preferredPositions) {
      ...
      if (overlayPoint.fitsInViewport) {
        ...
        const scrollableViewProperties = this.getScrollableViewProperties(
          element
        );
   ...
   */
  private getScrollableViewProperties(
    overlay: HTMLElement
  ): ScrollableViewProperties {
    ...
    return {
      isOriginClipped: this.isElementClipped(
        originBounds,
        scrollContainerBounds
      ),
      isOriginOutsideView: this.isElementOutsideView(
        originBounds,
        scrollContainerBounds
      ),
      isOverlayClipped: this.isElementClipped(
        overlayBounds,
        scrollContainerBounds
      ),
      isOverlayOutsideView: this.isElementOutsideView(
        overlayBounds,
        scrollContainerBounds
      )
    };
