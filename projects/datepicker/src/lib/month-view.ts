...
export class Md2MonthView implements AfterContentInit {
  ...
  private _init() {
    this._selectedDate = this._getDateInCurrentMonth(this.selected);
    this._todayDate = this._getDateInCurrentMonth([this._util.today()])[0];
  ...
  private _createWeekCells() {
    ...
    for (
      ...
      this._weeks[this._weeks.length - 1].push(
        new Md2CalendarCell(i + 1, dateNames[i], ariaLabel, enabled)
      );
    }
  }
  ...
  private _getDateInCurrentMonth(date: Date[]): number[] {
    ...
    date.forEach(d => {
      if (this._util.isSameMonthAndYear(d, this.activeDate)) {
        datesInCurrentMonth.push(this._util.getDate(d));
      }
    });
